...
void bst_delete(bst_node_t **tree, char key) {
    ...
    bst_node_t *replace_position;
    bst_node_t *replace_parent_position;

    while(position != NULL && key != position->key){
        parent = position;
        if(key < position->key){
            position = position->left;
        }else{
    ...
    if(position->left == NULL){
        replace_position = position->right;
    }else if (position->right == NULL){
        replace_position = position->left;
    }else{
        replace_parent_position = position;
        replace_position = position->left;
        bst_replace_by_rightmost(replace_parent_position,&replace_position);
    }
    if (parent == NULL){
        *tree = replace_position;
    }else{
        if (position->key < parent->key){
            parent->left = replace_position;
        }else{
            parent->right = replace_position;
        }
    }
    free(position);
    return;
